...
# Install script for Interactive Feedback MCP Go - Linux
# This script installs the MCP server to /usr/local/bin/

set -e

echo "🚀 Installing Interactive Feedback MCP Go for Linux..."
...
if [ ! -f "./interactive-feedback-mcp-linux-amd64" ]; then
...
cp interactive-feedback-mcp-linux-amd64 /usr/local/bin/interactive-feedback-mcp
